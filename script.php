 ...
 * @version 0.2b
 */
class com_breedableInstallerScript {
    ...
    public function preflight($type, $parent) {
        ...
        if (!$jversion->isCompatible($this->release)) {
            JFactory::getApplication()->enqueueMessage(JText::_('COM_BREEDABLE_IS_NOT_COMPATIBLE_JOOMLA_VERSION'), 'error');
            return false;
        }
    }
    ...
    private function installDb($parent) {
        ...
        if (function_exists('simplexml_load_file')) {
            ...
            $app->enqueueMessage(JText::_('COM_BREEDABLE_SIMPLEXML_LOAD_FILE_FUNCTION_DOES_NOT_EXISTS'));
        }
    }
            ...
            switch ($table['action']) {
                case 'add':
                    ...
                    if (!$this->existsTable($table['table_name'])) {
                        ...
                        try {
                            $db->execute();
                            $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_CREATING_TABLE_ACTION_COMPLETED_SUCCESFULLY', (string) $table['table_name']));
                            $table_added = true;
                        } catch (Exception $ex) {
                            $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_CREATING_TABLE_ACTION_AN_ERROR_OCCURRED', (string) $table['table_name'], $ex->getMessage()), 'error');
                        }
                    }
                    break;
                case 'change':
                    ...
                    if ($this->existsTable($table['old_name'])) {
                        try {
                            ...
                            $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_RENAMING_TABLE_ACTION_COMPLETED_SUCCESFULLY', $table['old_name'], $table['new_name']));
                        } catch (Exception $ex) {
                            $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_RENAMING_TABLE_ACTION_AN_ERROR_OCCURRED', $table['old_name'], $ex->getMessage()), 'error');
                        }
                    } else {
                        ...
                        if (!$this->existsTable($table['table_name'])) {
                            ...
                            try {
                                $db->execute();
                                $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_CREATING_TABLE_ACTION_COMPLETED_SUCCESFULLY', $table['table_name']));
                                $table_added = true;
                            } catch (Exception $ex) {
                                $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_CREATING_TABLE_ACTION_AN_ERROR_OCCURRED', $table['table_name'], $ex->getMessage()), 'error');
                            }
                        }
                    }
                    break;
                case 'remove':
                    ...
                    try {
                        ...
                        $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_DROPPING_TABLE_ACTION_COMPLETED_SUCCESSFULLY', $table['table_name']));
                    } catch (Exception $ex) {
                        $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_DROPPING_TABLE_ACTION_AN_ERROR_OCCURRED', $table['table_name'], $ex->getMessage()), 'error');
                    }
    ...
    private function generateCreateTableStatement($table) {
        ...
        if (isset($table->field)) {
            ...
            foreach ($indexes as $index) {
                $fields_definitions[] = JText::sprintf('COM_BREEDABLE_CREATE_TABLE_INDEX_SQL_STATEMENT', $db->quoteName((string) $index), $index);
            }

            $create_table_statement = JText::sprintf('COM_BREEDABLE_CREATE_TABLE_SQL_STATEMENT', $table['table_name'], implode(',', $fields_definitions));
        }
        ...
        if (isset($field['action'])) {
            switch ($field['action']) {
                case 'add':
                    ...
                    if ($result === MODIFIED) {
                        $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_ADDING_FIELD_ACTION_COMPLETED_SUCCESSFULLY', $field['field_name']));
                    } else if ($result !== NOT_MODIFIED) {
                        $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_ADDING_FIELD_ACTION_AN_ERROR_OCCURRED', $field['field_name'], $result), 'error');
                    }
                    break;
                case 'change':
                    ...
                    if (isset($field['old_name']) && isset($field['new_name'])) {
                        ...
                        if ($this->existsField($table_name, $field['old_name'])) {
                            $renaming_statement = JText::sprintf('COM_BREEDABLE_RENAME_COLUMN_SQL_STATEMENT', $table_name, $field['old_name'], $field['new_name'], $this->getFieldType($field));
                            $db->setQuery($renaming_statement);
                            try {
                                $db->execute();
                                $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_MODIFYING_FIELD_ACTION_COMPLETED_SUCCESSFULLY', $field['old_name']));
                            } catch (Exception $ex) {
                                $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_MODIFYING_FIELD_ACTION_AN_ERROR_OCCURRED', $field['field_name'], $ex->getMessage()), 'error');
                            }
                        } else {
                            $result = $this->addField($table_name, $field);
                            if ($result === MODIFIED) {
                                $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_MODIFYING_FIELD_ACTION_COMPLETED_SUCCESSFULLY', $field['field_name']));
                            } else if ($result !== NOT_MODIFIED) {
                                $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_MODIFYING_FIELD_ACTION_AN_ERROR_OCCURRED', $field['field_name'], $result), 'error');
                            }
                        }
                    } else {
                        $result = $this->addField($table_name, $field);
                ...
                case 'remove':
                    ...
                    if ($this->existsField($table_name, $field['field_name'])) {
                        $drop_statement = JText::sprintf('COM_BREEDABLE_DROP_COLUMN_SQL_STATEMENT', $table_name, $field['field_name']);
                        $db->setQuery($drop_statement);
                        try {
                            $db->execute();
                            $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_DROPPING_FIELD_ACTION_COMPLETED_SUCCESSFULLY', $field['field_name']));
                        } catch (Exception $ex) {
                            $app->enqueueMessage(JText::sprintf('COM_BREEDABLE_DROPPING_FIELD_ACTION_AN_ERROR_OCCURRED', $field['field_name'], $ex->getMessage()), 'error');
                        }
                    }
    ...
    private function generateAddFieldStatement($table_name, $field) {
        ...
        return JText::sprintf('COM_BREEDABLE_ADD_COLUMN_SQL_STATEMENT', $table_name, $column_declaration);
    }
    ...
    private function generateChangeFieldStatement($table_name, $field) {
        $column_declaration = $this->generateColumnDeclaration($field);
        return JText::sprintf('COM_BREEDABLE_CHANGE_COLUMN_TYPE_SQL_STATEMENT', $table_name, $column_declaration);
    }
    ...
    private function generateColumnDeclaration($field) {
        ...
        return JText::sprintf('COM_BREEDABLE_CREATE_TABLE_COLUMN_DECLARATION_SQL_STATEMENT', $col_name, $data_type, $default_value, $other_data, $comment_value);
    }
    ...
    private function needsToUpdate($table_name, $field) {
        ...
        $query = JText::sprintf('COM_BREEDABLE_SHOW_COLUMNS_TABLE_SQL_STATEMENT', $table_name, $db->quote((string) $field['field_name']));
        $db->setQuery($query);
