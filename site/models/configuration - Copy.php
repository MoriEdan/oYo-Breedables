...
*/
	public function configure( $data = null ) {
		...
		if($data['mode'] == "sibling") {
			...
			//$query1->join('LEFT', $db->quoteName('#__breedable') . ' as mother ON mother.mother_id=c.mother_id');
			
			// Joing with the father
			//$query1->join('LEFT', $db->quoteName('#__breedable') . ' as father ON father.father_id=c.father_id');
...
//var_dump($db->replacePrefix( (string) $query1 ) );
		...
		if($data['mode'] == "father") {
			...
			if( $data['owner_name'] == "Revolution Perenti")
			{
			...
			if( $data['owner_name'] == "Revolution Perenti")
			{
				$query4->join('LEFT', $db->quoteName('#__breedable_queued') . ' as queued ON queued.breedable_id=f.id');
			}
			//pregnancy
...
*/
		...
		if($data['mode'] == "mother") {
			...
			// Joing with the sibling
			//$query5->join('LEFT', $db->quoteName('#__breedable') . ' as sibling ON sibling.mother_id=c.id');
